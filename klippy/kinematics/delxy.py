...
class DelXYKinematics:
    def __init__(self, toolhead, config):
        ...
        self.rails = [ stepper.PrinterRail(stepper_configs[0], need_position_minmax = False), 
                       stepper.PrinterRail(stepper_configs[0], need_position_minmax = False), 
        ...
        self.tower_dists = [ stepper_configs[0].getfloat('tower_pos', minval=-self.arm_lengths[0]),
                        stepper_configs[1].getfloat('tower_pos', maxval=self.arm_lengths[1])]
        self.height_offset = config.getfloat('delxy_height', above=0.)
        self.abs_endstop_locations = [rail.get_homing_info().position_endstop for rail in self.rails]
        self.abs_endstop_locations[0] = self.abs_endstop_locations[0] + math.sqrt(self.arm2[0] - self.tower_dists[0] ** 2) + self.height_offset
        self.abs_endstop_locations[1] = self.abs_endstop_locations[1] + math.sqrt(self.arm2[1] - self.tower_dists[1] ** 2) + self.height_offset
        ...
        self.home_position = self._calc_delxy_cartesian_pos(self.abs_endstop_locations)

        # Allocate stepper solvers
        self.rails[0].setup_itersolve('delxy_stepper_alloc', self.arm2[0], self.tower_dists[0])
        self.rails[1].setup_itersolve('delxy_stepper_alloc', self.arm2[1], self.tower_dists[1])
        ...
        self.rails[2].set_max_jerk(min(self.max_z_velocity, max_halt_velocity), self.max_z_accel)
    ...
    def _calc_delxy_cartesian_pos(self, spos):
        d = math.sqrt((self.tower_dists[1] - self.tower_dists[0]) ** 2 + (spos[1] - spos[0]) ** 2)
        a = (self.arm2[0] - self.arm2[1] + d ** 2) / (2 * d)
        h = math.sqrt(self.arm2[0] - a ** 2)
        x2 = self.tower_dists[0] + a * (self.tower_dists[1] - self.tower_dists[0]) / d
        ...
        # Pick the set [x3[0], y3[0]] or [x3[1], y3[1]] which gives a y coordinate < both spos[0] and spos[1]
